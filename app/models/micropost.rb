class Micropost < ApplicationRecord
	...
	def self.search_by_parameter_sort_by_like_with_pagination(search_version, keyword, micropost_liked_ids, params_page)
		if keyword.empty?
			where(id: micropost_liked_ids).page(params_page)
		else
			search_by_parameter_sort_by_like(search_version, keyword, micropost_liked_ids).page(params_page)
		end
	end
	...
	def self.search_by_parameter_sort_by_like_and_video_type_with_pagination(search_version, keyword, micropost_liked_ids, video_type, params_page)
		if keyword.empty?
			where('id IN (?) AND video_type = ?', micropost_liked_ids, video_type).page(params_page)
		else
			search_by_parameter_sort_by_like_and_video_type(search_version, keyword, micropost_liked_ids, video_type).page(params_page)
		end
	end
